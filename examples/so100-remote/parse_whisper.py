    ...
    if event["type"] == "INPUT":
        if event["id"] == "text":
            ...
            if "grab " in text:
                ...
                text = "Given the prompt: grab the sausage. Output the bounding boxes for the given object"
                node.send_output(
                    "text", pa.array([text]), {"image_id": "image", "action": "grab"}
                )
                time.sleep(4.0)

                text = "Given the prompt: put it in the black cooking grill. Output the bounding boxes for the given object"
                node.send_output(
                ...
                text = "Given the prompt: put it in the slice of bread. Output the bounding boxes for the given object"
                node.send_output(
                    "text", pa.array([text]), {"image_id": "image", "action": "release"}
                )
